...
COMMENT ON FUNCTION public.create_default_lists_for_user IS 'Creates the 3 default lists (Favorites, Liked, Want to Go) for a user. Guarded so anon callers are blocked and authenticated callers can only act on their own user_id; trigger/service-role contexts are allowed.';
